...
public class FFTDemo {
    ...
    public static void main(String[] arguments) throws IOException {
        if (arguments.length != 4) {
            throw new IllegalArgumentException("Usage: java Demo [myId] [otherIP] [n] [CRT/SPDZ]");
        }
        ...
        String scheme = arguments[3];
        Instant start;
        List<OpenComplex> results;

        if (scheme.equalsIgnoreCase("crt")) {
            ...
            SpdzResourcePool rpRight = new SpdzResourcePoolImpl(myId, noParties,
                    ...
                    AesCtrDrbg::new);
            CRTDataSupplier<SpdzResourcePool, SpdzResourcePool> dataSupplier = new CRTSemiHonestDataSupplier<>(null);
            ...
            results = sce
                    .runApplication(new FFTApp(n),
                            rp, network, Duration.ofMinutes(30));

        } else if (scheme.equalsIgnoreCase("spdz")) {
            ...
            BatchedProtocolEvaluator<SpdzResourcePool> evaluator =
                    new BatchedProtocolEvaluator<>(EvaluationStrategy.SEQUENTIAL.getStrategy(), suite,
                            maxBatchSize);
